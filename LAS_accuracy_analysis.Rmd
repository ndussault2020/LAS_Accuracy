---
title: "Accuracy of of LAS - Analysis"
...
#Actual LAS scores from SRTR
las_scores <- read_csv("./Data/lu_alloc_scores.csv")
...
load("./Data/calib_bootstraps.Rdata")
```
...
#      file = "bootstrap_KM_deciles.Rdata")

load("./Data/bootstrap_KM_deciles.Rdata")
```
...
aspect_ratio = 1.5
invisible(ggsave(filename = "combined_figure_WLAUC.jpg", plot = combined_figure_WLAUC, path = "./Graphs", height = 7, width = 7*aspect_ratio ))
...
aspect_ratio = 1.5
invisible(ggsave(filename = "combined_figure_PTAUC.jpg", plot = combined_figure_PTAUC, path = "./Graphs", height = 7, width = 7*aspect_ratio ))
...
aspect_ratio = 1.5
invisible(ggsave(filename = "combined_figure_calibration.jpg", plot = combined_figure_calibration, path = "./Graphs", height = 7, width = 7*aspect_ratio ))
...
#save(observed_LAS_samples, file = "LAS_samples.RData")

load("./Data/LAS_samples.RData")
```
...
LAS_calib <- output %>%
  ...
  mutate(low_ci = mean_observed_LAS - 1.96*se_observed_LAS,
         ...
         error = average_LAS - mean_observed_LAS) 



average_calibration_error <- mean(LAS_calib$average_LAS - LAS_calib$mean_observed_LAS)

se_avg_calib_error <- sd(LAS_calib$average_LAS - LAS_calib$mean_observed_LAS)
...
ggsave("./Graphs/LAS_Calibration.jpg")



#FInd slope and intercept of graph
lm.las <- lm(LAS_calib$average_LAS ~ LAS_calib$mean_observed_LAS) # create linear model
summary(lm.las) # summary

#Average error across the first 10 percentiles (i.e. first decile) and across the 99-100th percentiles (i.e. last decile)
error_perc_1_10 <-  LAS_calib %>%slice(1:10) %>%summarise(a = mean(error))
error_perc_91_100 <-  LAS_calib %>% slice(91:100) %>%summarise(a = mean(error))

#average error across all
error_avg <-  LAS_calib %>%summarise(a = mean(error))
ob_avg <-  LAS_calib %>%summarise(a = mean(mean_observed_LAS))
pred_avg <-  LAS_calib %>%summarise(a = mean(average_LAS))


#Number of percentiles whose LAS prediction is > than the observed
LAS_calib%>%transmute(flag = ifelse(error>0,1,0)) %>% filter(flag == 1) %>%nrow()
...
#Compare calculated LAS scores to actual LAS scores
```{r actualLAS}

actualLAS <- las_scores %>%
  #filter(CANHX_LAS_SCORE_CALC !=0 & !is.na(CANHX_LAS_SCORE_CALC)) %>%
  group_by(PX_ID) %>% arrange(CANHX_LAS_SCORE_BEGIN_DT)%>%
  mutate(first_day = min(CANHX_LAS_SCORE_BEGIN_DT)) %>%         #filter to only the first day forms, i.e. LAS at listing
  filter(CANHX_LAS_SCORE_BEGIN_DT == first_day) %>%
  ungroup()%>%
  mutate(PX_ID = as.integer(PX_ID))

cad <- candidates%>%
  left_join(actualLAS, by = "PX_ID")

summary(lm(formula = cad$LAS ~ cad$CANHX_LAS_SCORE_MATCH))
summary(lm(formula = cad$CANHX_LAS_SCORE_MATCH ~ cad$LAS))
ggplot(cad, aes(x = LAS, y=CANHX_LAS_SCORE_MATCH)) + geom_point()
ggplot(cad, aes(x = CANHX_LAS_SCORE_MATCH, y=LAS)) + geom_point()
cor(cad$LAS, cad$CANHX_LAS_SCORE_MATCH, use = "complete.obs")
...
## Check that the R^2 is not sensitive to the number of samples, i.e. that with 1,000 bootstraps or 20,000 bootstraps the R^2 will be roughly the same
## Check the sensitivity analysis of too sick to transplant as deaths
